...
from flash_attn_rocm.flash_attention import _get_block_size, unpad_input, pad_input
from flash_attn_rocm.flash_attention import FlashAttention
...
def generate_random_padding_mask(max_seqlen, batch_size, device, mode='random'):
    ...
    if mode == 'full':
        ...
        lengths = torch.randint(max(1, max_seqlen - 20),
    ...
    padding_mask = repeat(torch.arange(
        max_seqlen, device=device), 's -> b s', b=batch_size) < lengths
    return padding_mask
...
def generate_qkv(x, Wqkv, nheads, query_padding_mask=None, key_padding_mask=None,
    ...
    if qkvpacked:
        ...
        qkv = rearrange(torch.stack([q, k, v], dim=2),
                        'b s t (h d) -> b s t h d', h=nheads)
        if query_padding_mask is not None:
            def dqkv_pad_fn(dqkv_unpad): return rearrange(
                pad_input(rearrange(dqkv_unpad, 'nnz t h d -> nnz (t h d)'),
                          indices_q, batch_size, seqlen),
        ...
        if key_padding_mask is not None:
            def dkv_pad_fn(dkv_unpad): return rearrange(
                pad_input(rearrange(dkv_unpad, 'nnz t h d -> nnz (t h d)'),
                          indices_k, batch_size, seqlen),
        ...
        if key_padding_mask is not None:
            def dk_pad_fn(dk_unpad): return rearrange(
                pad_input(rearrange(dk_unpad, 'nnz h d -> nnz (h d)'),
                          indices_k, batch_size, seqlen),
                'b s (h d) -> b s h d', h=nheads
            )
        else:
...
def attention_ref(q, k, v, query_padding_mask=None, key_padding_mask=None, dropout_p=0.0,
    ...
    if causal:
        causal_mask = torch.triu(torch.ones(
            seqlen_q, seqlen_k, dtype=torch.bool, device=q.device), 1)
...
def test_flash_attn(seqlen, d, dropout_p, causal, dtype, batch_size, nheads):
    device = 'cuda'
    # set seed
    torch.random.manual_seed(0)
    # Set smaller batch size so it would trigger num_splits > 1
    x = torch.randn(batch_size, seqlen, nheads * d,
                    device=device, dtype=dtype, requires_grad=True)
    Wqkv = torch.nn.Linear(nheads * d, 3 * nheads * d,
    ...
    output, _ = flash_attn(qkv)

    output_ref, attn_ref = attention_qkvpacked_ref(
        qkv, dropout_p=dropout_p, causal=causal)
    output_pt, attn_pt = attention_qkvpacked_ref(
        qkv, dropout_p=dropout_p, causal=causal, upcast=False, reorder_ops=True)
    ...
    print(
        f'Attention Pytorch max diff: {(attn_pt - attn_ref).abs().max().item()}')

    g = torch.randn_like(output)
    dqkv_ref, = torch.autograd.grad(output_ref, qkv, g)
    dqkv_pt, = torch.autograd.grad(output_pt, qkv, g)
    dqkv, = torch.autograd.grad(output, qkv, g)
    print(
        f'dQ max diff: {(dqkv[:, :, 0] - dqkv_ref[:, :, 0]).abs().max().item()}')
    print(
        f'dK max diff: {(dqkv[:, :, 1] - dqkv_ref[:, :, 1]).abs().max().item()}')
    print(
        f'dV max diff: {(dqkv[:, :, 2] - dqkv_ref[:, :, 2]).abs().max().item()}')
    ...
    print(
        f'dQKV Pytorch mean diff: {(dqkv_pt - dqkv_ref).abs().mean().item()}')

    # Check that FlashAttention's numerical error is at most twice the numerical error
    # of a Pytorch implementation.
    assert (output - output_ref).abs().max().item() <= 2 * \
        (output_pt - output_ref).abs().max().item()

    # Error for dK and dV could be a bit higher if we're splitting along seqlen_q dimension
    assert (dqkv - dqkv_ref).abs().max().item() <= 4 * \
        (dqkv_pt - dqkv_ref).abs().max().item()
